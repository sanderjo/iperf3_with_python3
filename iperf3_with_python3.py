...
def run_speedtest(servername):
	for iptype in [4,6]:
		...
		for _ in range(5): # max 5 tries
			...
			#print(ports)
			cmd = "iperf3 -p {port} -t3 -{ipv46} --connect-timeout 300 --format m -c {server}".format(port=serverport, ipv46=iptype, server=servername)
			print(cmd)
			...
			if err:
				...
				if "unable to send control message: Bad file descriptor" in err:
					# network problem. No IPv6?
					break
			else:
				for thisline in output.split("\n"):
					print("SPEED:", thisline.split()[-4], "Mbps") if "sender" in thisline else None
				break
...
servers = ["ams.speedtest.clouvider.net", "la.speedtest.clouvider.net", "fra.speedtest.clouvider.net", "ash.speedtest.clouvider.net", "speedtest.uztelecom.uz", "proof.ovh.net"]
for server in servers:
	server_pingtime = pingtime(server)
	print(server, server_pingtime)
